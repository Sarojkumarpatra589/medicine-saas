...
import { FiCalendar, FiUsers, FiXSquare, FiPlus, FiSettings, FiCheckCircle, FiBell, FiSun, FiMoon, FiRefreshCw } from "react-icons/fi";
  ...
  return (
    <>
      <style>{`
        ...
        .nh-icon-btn:hover { background: #f3f4f6; color: #374151; }
        .nh-notif-dot {
        ...
        .nh-avatar-wrap { position: relative; flex-shrink: 0; cursor: pointer; }
        .nh-avatar {
          width: 34px; height: 34px; border-radius: 50%;
          object-fit: cover; border: 2px solid #c7d2fe; display: block;
        }
        .nh-online {
      ...
      <header className="nh-bar box_shadow">

        {/* Title */}
        <span className="nh-title mx-3">Staff Dashboard</span>

        <div className="nh-spacer" />

        {/* + New Appointment */}
        <button className="nh-btn-primary">
          <FiPlus size={16} />
          New Appointment
        </button>

        {/* Schedule Availability */}
        <button className="nh-btn-outline">
          <FiCalendar size={15} />
          Schedule Availability
        </button>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ position: "relative" }}>
          <button onClick={() => setFilterOpen(o => !o)} onBlur={() => setTimeout(() => setFilterOpen(false), 150)}
            style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 8, padding: "5px 12px", fontSize: 13, color: "#374151", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontWeight: 500 }}>
            {filter} <ChevronDown />
          </button>
          {filterOpen && (
            <div style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", background: "#fff", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.12)", zIndex: 100, minWidth: 130, overflow: "hidden" }}>
              {["Today", "Tomorrow", "This Week"].map(v => (
                <button key={v} onClick={() => { setFilter(v); setFilterOpen(false); }}
                  style={{ display: "block", width: "100%", padding: "9px 16px", background: filter === v ? "#f3f4f6" : "none", border: "none", textAlign: "left", fontSize: 13, color: filter === v ? "#3b51d8" : "#374151", cursor: "pointer", fontWeight: filter === v ? 600 : 400 }}>{v}</button>
              ))}
            </div>
          )}
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <img src="https://i.pravatar.cc/150?img=12" alt="Andrew Billard"
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
          <CalendarIcon /><span style={{ fontSize: 13, color: "#6b7280" }}>Monday, 31 Mar 2025</span>
          <span style={{ margin: "0 4px", color: "#d1d5db" }}>·</span>
          <ClockIcon /><span style={{ fontSize: 13, color: "#6b7280" }}>06:30 PM</span>
        </div>
      </div>
      ...
      <button style={{ background: "#3b51d8", color: "#fff", border: "none", borderRadius: 10, padding: "12px 0", fontSize: 14, fontWeight: 600, cursor: "pointer", width: "100%", letterSpacing: "0.01em", transition: "background 0.15s" }}
        onMouseEnter={e => e.currentTarget.style.background = "#2e42b8"}
        onMouseLeave={e => e.currentTarget.style.background = "#3b51d8"}>
        Start Appointment
      </button>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <button style={{ background: "#111827", color: "#fff", border: "none", borderRadius: 10, padding: "10px 0", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, transition: "background 0.15s" }}
          onMouseEnter={e => e.currentTarget.style.background = "#1f2937"}
          onMouseLeave={e => e.currentTarget.style.background = "#111827"}>
          <ChatIcon /> Chat Now
        </button>
        <button style={{ background: "#fff", color: "#374151", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "10px 0", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, transition: "background 0.15s" }}
...
function AppointmentsChart() {
  ...
  const areaPoints = [`${getX(0)},${padT + chartH}`, ...data.map((d, i) => `${getX(i)},${getY(d.total)}`), `${getX(n - 1)},${padT + chartH}`].join(" ");
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <h5 style={{ margin: 0, fontWeight: 700, fontSize: 17, color: "#111827" }}>Appointments</h5>
        <div style={{ position: "relative" }}>
          <button onClick={() => setDropdownOpen(o => !o)} onBlur={() => setTimeout(() => setDropdownOpen(false), 150)}
            style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 8, padding: "5px 12px", fontSize: 13, color: "#374151", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontWeight: 500 }}>
            {chartView} <ChevronDown />
          </button>
          {dropdownOpen && (
            <div style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", background: "#fff", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.12)", zIndex: 100, minWidth: 120, overflow: "hidden" }}>
              {["Weekly", "Monthly", "Yearly"].map(v => (
                <button key={v} onClick={() => { setChartView(v); setDropdownOpen(false); }}
                  style={{ display: "block", width: "100%", padding: "9px 16px", background: chartView === v ? "#f3f4f6" : "none", border: "none", textAlign: "left", fontSize: 13, color: chartView === v ? "#3b51d8" : "#374151", cursor: "pointer", fontWeight: chartView === v ? 600 : 400 }}>{v}</button>
              ))}
            </div>
          )}
        </div>
      ...
      <div style={{ flex: 1, position: "relative" }}>
        {tooltip && (
          <div style={{ position: "absolute", background: "#1f2937", color: "#fff", padding: "7px 11px", borderRadius: 8, fontSize: 12, pointerEvents: "none", whiteSpace: "nowrap", left: tooltip.x, top: tooltip.y, transform: "translate(-50%, -115%)", zIndex: 200 }}>
        ...
        <svg viewBox={`0 0 ${svgW} ${svgH}`} preserveAspectRatio="xMidYMid meet" style={{ width: "100%", height: "100%", display: "block" }}>
          <defs>
          ...
          {data.map((d, i) => {
            ...
            return (<rect key={i} x={bx} y={by} width={barW} height={bh} fill="url(#sec2BarGrad)" rx="4"
          ...
          <polyline points={completedPoints} fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
          {data.map((d, i) => {
          ...
          {data.map((d, i) => (
            <text key={`xl-${i}`} x={getX(i)} y={svgH - 8} fontSize="12" fill="#9ca3af" textAnchor="middle" fontWeight="500">{d.month}</text>
          ))}
    ...
    <div style={{ background: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div className="p-3 p-md-4">
        ...
        <Row className="g-3 mb-3">
          {[
            { label: "Total Appointments", value: "658", badge: "+95%", badgeBg: "#22c55e", icon: <FiCalendar size={20} />, iconColor: "#4840BB", iconBorder: "#c7d2fe", iconBg: "#f5f3ff", bars: [28, 38, 44, 36, 50, 42, 30], barColor: "#4840BB", trend: "+21%", trendUp: true, trendColor: "#22c55e" },
            { label: "Online Consultations", value: "125", badge: "-15%", badgeBg: "#ef4444", icon: <FiUsers size={20} />, iconColor: "#ef4444", iconBorder: "#fecaca", iconBg: "#fff5f5", bars: [32, 44, 36, 50, 38, 46, 28], barColor: "#f97316", trend: "+21%", trendUp: false, trendColor: "#22c55e" },
            { label: "Cancelled Appointments", value: "35", badge: "+45%", badgeBg: "#22c55e", icon: <FiXSquare size={20} />, iconColor: "#22c55e", iconBorder: "#bbf7d0", iconBg: "#f0fdf4", bars: [20, 28, 22, 36, 30, 40, 34], barColor: "#22c55e", trend: "+31%", trendUp: true, trendColor: "#22c55e" },
            { label: "Completed Appointments", value: "512", badge: "+22%", badgeBg: "#22c55e", icon: <FiCheckCircle size={20} />, iconColor: "#F59E0B", iconBorder: "#fde68a", iconBg: "#fffbeb", bars: [24, 34, 28, 42, 36, 44, 32], barColor: "#F59E0B", trend: "+14%", trendUp: true, trendColor: "#22c55e" },
          ].map((card, i) => (
            <Col key={i} xs={12} sm={6} xl={3}>
              <Card className="border-0 box_shadow h-100" style={{border: "1px solid #f1f5f9" }}>
                <Card.Body className="p-3">
                  <div className="d-flex justify-content-between align-items-start mb-2">
                    <span style={{ fontSize: 13, color: "#64748b", fontWeight: 500, lineHeight: 1 }}>{card.label}</span>
                    <div style={{ width: 40, height: 40, borderRadius: 8, border: `1.5px solid ${card.iconBorder}`, backgroundColor: card.iconBg, color: card.iconColor, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginLeft: 8 }}>{card.icon}</div>
                  </div>
        ...
        {/* SECTION 2 — Upcoming + Chart */}
        <style>{`
          .sec2-grid { display: grid; grid-template-columns: 340px 1fr; gap: 20px; margin-bottom: 1rem; align-items: stretch; }
          .sec2-card { background: #ffffff;0 2px 10px rgba(0,0,0,0.05); padding: 24px; }
          @media (max-width: 992px) { .sec2-grid { grid-template-columns: 1fr; } }
          @media (max-width: 480px) { .sec2-card { padding: 16px; } }
        `}</style>
        <div className="sec2-grid ">
          <div className="sec2-card box_shadow"><UpcomingCard /></div>
          <div className="sec2-card box_shadow" style={{ display: "flex", flexDirection: "column" }}><AppointmentsChart /></div>
        </div>

        {/* SECTION 3 — Mini Square Stats */}
        <Row className="g-3 mb-3">
          {miniStats.map((stat, index) => (
            <Col xs={6} sm={4} md={3} lg={2} key={index}>
              <div style={{ width: "100%", aspectRatio: "1/1", background: "#fff", borderRadius: 14, padding: 14, display: "flex", flexDirection: "column", justifyContent: "space-between", }} className="box_shadow">
                <div style={{ width: 44, height: 44, borderRadius: 10, background: stat.iconBg, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}>
        ...
        {/* SECTION 4 — Recent Appointments Table */}
        <Card className="border-0 box_shadow mb-4">
          <Card.Body className="p-0">
            <div className="d-flex justify-content-between align-items-center p-4 pb-3">
              ...
              <Dropdown>
                <Dropdown.Toggle variant="light" size="sm" className="border-0 bg-white" style={{ border: "1px solid #e5e7eb" }}>{weekFilter}</Dropdown.Toggle>
                    ...
                    <tr key={appt.id} style={{ borderBottom: index < appointments.length - 1 ? "1px solid #E5E7EB" : "none" }}>
                      <td className="py-3 ps-4 border-0">
                        <div className="d-flex align-items-center">
                          <img src={appt.avatar} alt={appt.name} className="rounded-circle me-3" style={{ width: 40, height: 40, objectFit: "cover" }} />
                          <div>
                      ...
                      <td className="py-3 border-0">
                        <Badge bg={appt.statusVariant} className="px-3 py-2 fw-normal rounded-pill" style={{ fontSize: "0.75rem" }}>{appt.status}</Badge>
                      </td>
                      <td className="py-3 fw-semibold text-dark border-0">{appt.fees}</td>
                      <td className="py-3 pe-4 border-0">
                        <div className="d-flex gap-2 justify-content-end">
                          <Button variant="light" size="sm" className="border-0 bg-white p-2" style={{ border: "1px solid #E5E7EB", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <i className="bi bi-printer text-muted"></i>
                          </Button>
                          <Dropdown>
                            <Dropdown.Toggle variant="light" size="sm" className="border-0 bg-white p-2" id={`dropdown-${appt.id}`} style={{ border: "1px solid #E5E7EB", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <Row className="g-3 mb-4">
          <Col xs={12} sm={6} md={4}>
            <Card className="border-0 h-100 box_shadow">
              <Card.Body className="p-3 p-md-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h6 className="fw-bold mb-0" style={{ color: "#1e293b", fontSize: 14 }}>Availability</h6>
                  <select className="form-select form-select-sm" style={{ width: "auto", fontSize: 12, color: "#475569", borderColor: "#e2e8f0", borderRadius: 8, maxWidth: 120 }} value={clinicFilter} onChange={e => setClinicFilter(e.target.value)}>
                    <option>Trustcare Clinic</option>
                    <option>City Health Center</option>
                  </select>
                </div>
                {availability.map(row => (
                  <div key={row.day} className="d-flex justify-content-between align-items-center" style={{ padding: "8px 0", borderBottom: "1px solid #f1f5f9" }}>
                ...
                <Button variant="outline-secondary" className="w-100 mt-3 fw-semibold" style={{ fontSize: 13, borderColor: "#e2e8f0", borderRadius: 8, color: "#475569" }}>Edit Availability</Button>
              </Card.Body>
            </Card>
          </Col>

          <Col xs={12} sm={6} md={4}>
            <Card className="border-0 box_shadow h-100">
                  ...
                  <div className="d-flex align-items-center justify-content-center gap-3 mt-1">
                    {appointmentStats.map(d => (
                      <div key={d.name} className="d-flex align-items-center gap-1" style={{ fontSize: 12.5, color: "#6c757d" }}>
                        <span style={{ width: 9, height: 9, borderRadius: "50%", backgroundColor: d.color, display: "inline-block", flexShrink: 0 }} />{d.name}
                      </div>
                    ))}
                  </div>
            ...
            <Card className="border-0 box_shadow h-100">
              <Card.Body className="p-3 p-md-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <select className="form-select form-select-sm" style={{ width: "auto", fontSize: 12, color: "#475569", borderColor: "#e2e8f0", borderRadius: 8 }} value={patientFilter} onChange={e => setPatientFilter(e.target.value)}>
                    <option>Weekly</option>
                    <option>Monthly</option>
                    <option>Yearly</option>
                  </select>
                ...
                {topPatients.map(p => (
                  <div key={p.name} style={{ display: "grid", gridTemplateColumns: "36px 1fr auto", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <span style={{ backgroundColor: "#eff6ff", color: "#3b82f6", border: "1px solid #bfdbfe", borderRadius: 6, padding: "3px 9px", fontSize: 11.5, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>{p.appointments}</span>
                  </div>
                ))}
              </Card.Body>
            </Card>
